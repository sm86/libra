...
module SimpleSortedLinkedList {
    ...
    public fun add_node<T: copyable>(account: &signer, key: T, prev_node_address: address) acquires Node {
        ...
        let key_lcs_bytes = BCS::to_bytes(&key);
        let cmp_with_prev = Compare::cmp_lcs_bytes(&key_lcs_bytes, &BCS::to_bytes(&prev_key));
        let cmp_with_next = Compare::cmp_lcs_bytes(&key_lcs_bytes, &BCS::to_bytes(&next_key));
    ...
    public fun find<T: copyable>(key: T, head_address: address): (bool, address) acquires Node {
        ...
        while (next_node_address != head_address) {
            ...
            let next_key_lcs_bytes = BCS::to_bytes(&next_node_key);
